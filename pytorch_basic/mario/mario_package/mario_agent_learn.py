...
class Mario(Mario):
    def __init__(self, state_dim, action_dim, memory_maxlen, batch_size):
        super().__init__(state_dim, action_dim, memory_maxlen, batch_size)
        self.gamma = 0.9

    def td_estimate(self, state, action):

        # Q for (batch,action_space)
        current_state_Q = self.net(state, model="online")
...
class Mario(Mario):
    ...
    def update_Q_online(self, td_estimate, td_target):
        # loss between current and target TD
        loss = self.loss_fn(td_estimate, td_target)
        # reset grad on each weight to 0, otherwise accumulate on batch
...
class Mario(Mario):
    def __init__(self, state_dim, action_dim, memory_maxlen, batch_size,
                 learn_rate, exploration_rate_decay, save_dir, save_every):
        super().__init__(state_dim, action_dim,
                         memory_maxlen, batch_size, learn_rate)
    ...
    def save(self):
        ...
        torch.save(
            # exploration rate changing during training
